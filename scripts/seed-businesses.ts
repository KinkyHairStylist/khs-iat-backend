...
async function seed() {
  ...
  try {
    await AppDataSource.query(`DELETE FROM booking_days WHERE "businessId" IN (SELECT id FROM businesses WHERE "businessName" IN (${businessNames.map(n => `'${n}'`).join(',')}))`);
  } catch (e) {}
  try {
    await AppDataSource.query(`DELETE FROM staff WHERE email LIKE 'staff%@business%.com'`);
  } catch (e) {}
  try {
    await AppDataSource.query(`DELETE FROM "Service" WHERE "businessId" IN (SELECT id FROM businesses WHERE "businessName" IN (${businessNames.map(n => `'${n}'`).join(',')}))`);
  } catch (e) {}
  try {
    await AppDataSource.query(`DELETE FROM businesses WHERE "businessName" IN (${businessNames.map(n => `'${n}'`).join(',')})`);
  } catch (e) {}
  try {
    await AppDataSource.query(`DELETE FROM "user" WHERE email LIKE 'owner%@business%.com'`);
  } catch (e) {}
  ...
  for (let i = 0; i < 20; i++) {
    ...
    const businessResult = await AppDataSource.query(
      ...
      [
        businessNames[i],
        `Welcome to ${businessNames[i]}! We offer premium beauty and wellness services in ${cities[cityIndex].name}. Our skilled professionals are dedicated to making you look and feel your best.`,
        ownerId,
    ...
    // Create 5 services for each business
    for (let k = 0; k < 5; k++) {
      const serviceType = serviceTypes[(i + k) % serviceTypes.length];
  ...
  console.log('\n=== Seeding Complete ===');
  console.log('Created:');
  console.log('- 20 Business Owners (all verified with password: password123)');
  console.log('- 20 Businesses (all APPROVED)');
  console.log('- 100 Staff Members (5 per business)');
  console.log('- 100 Services (5 per business)');
  console.log('- 140 Booking Days (7 per business)');
